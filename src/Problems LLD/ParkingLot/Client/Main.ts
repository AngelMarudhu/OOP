...
import { ParkingSpotType } from "../ParkingSpot/ParkingSpotType.js";

class Display {
  constructor(private parking: ParkingLots) {}

  showAvailable(): void {
    for (let lev of this.parking.level) {
      console.log(
        `${lev.totalNumber}: and ${lev.freeSpots()} spots available buddy`
      );
    }
  }
}
