...
function genericsss(arg: any): any {
  return arg;
}
...
///////// If you want to handle a single string (or any type that has a length property) using generics
//// there is bunch of solution for this for example you can mention explicity the data type besides the T argument like
...
function genericss<T extends Names, length extends Lengths>(
  arg: T,
  lengthVal: length
): T {
  //   console.log(arg.length);
...
function greet(name: Names): void {
...
let check = new Gens<number, string>();
check.Push(12);
check.Push(14);
check.Push("Marudhu");
...
class SubGen<T, U> extends Gens<T, U> {
  constructor() {
    super();
  }
}
